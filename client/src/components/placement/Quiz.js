...
import { placement } from "../../actions/action";
...
export default function Placement() {
  ...
  const getQuestions = async (category) => {
    ...
    let res = await placement({
      subject: "math",
    });

    const quizQuestions = res.data.questions;
    const answer_choices = res.data.answer_choices;
    ...
    setTimeout(() => {
      for (let i = 0; i < quizQuestions.length; i++) {
        let real_choices = answer_choices[i].split(", ");

        console.log("CHOICEEESSS-> ", real_choices);
          ...
          <CustomCard
            ...
            text={
              <Box display="flex" justifyContent="center">
                <CustomButton onClick={getQuestions}>Start Test</CustomButton>
                <Dialog
                  open={openForm}
                  onClose={() => {
                    setOpenForm(false);
